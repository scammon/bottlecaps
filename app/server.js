...
// system, not a lightweight per-app login -- see git history). Core also
// exposes plain JSON auth endpoints (auth/app-signup, auth/login,
// auth/refresh) scoped to this app via X-Client-Id, confirmed against a
// real signup+login+refresh round trip:
//   - RS256, verifiable via Core's JWKS, issuer https://core.dailey.cloud
//   - the per-user identifier is `sub` (stable) -- NOT `tenant`, which here
//     is actually a "Library" grouping tied to the whole Dailey account,
//     not the individual bottlecaps user
//   - `aud` is the app's UUID (not the "bottlecaps" slug) and, unlike
//     `app_slug`, survives a token refresh -- app_slug does NOT, it's
//     absent from refreshed tokens, so aud is the only claim safe to gate
//     on for every token this app will ever see
// The frontend talks to Core directly (browser -> core.dailey.cloud, CORS
// already opened for this app's origin) and hands us a Bearer token --
// this server never sees a password and never calls Core to mint one.
const CORE_ISSUER = 'https://core.dailey.cloud';
const CORE_JWKS = createRemoteJWKSet(new URL(`${CORE_ISSUER}/.well-known/jwks.json`));
const DAILEY_APP_ID = process.env.DAILEY_APP_ID;
...
// --- Auth --------------------------------------------------------------
//
// The frontend gets tokens directly from Core (auth/app-signup, auth/login,
// auth/refresh -- browser to core.dailey.cloud, CORS opened for this app's
// origin) and sends the access_token as a normal Authorization: Bearer
// header. This server only ever verifies; it never sees a password and
// never talks to Core itself.
async function verifyDaileyToken(token) {
  const { payload } = await jwtVerify(token, CORE_JWKS, {
    issuer: CORE_ISSUER,
    audience: DAILEY_APP_ID,
  });
  if (!payload.sub) {
    throw new Error('token has no sub claim');
  }
  return {
    tenant: String(payload.sub),
